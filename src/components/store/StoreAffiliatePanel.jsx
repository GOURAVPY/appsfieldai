...
export default function StoreAffiliatePanel({ open, onClose, marketplaceId, affiliateSettings, listings = [], storeBaseUrl, brandColor = "#f97316" }) {
  ...
  useEffect(() => { if (open) load(); /* eslint-disable-next-line */ }, [open, marketplaceId]);
      ...
      <div className="w-full max-w-md h-full bg-background border-l border-border/40 flex flex-col" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-5 h-16 border-b border-border/40 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold leading-tight">Affiliate Program</p>
              <p className="text-[11px] text-muted-foreground">Earn commission promoting products</p>
            </div>
          </div>
          <button onClick={onClose} className="text-muted-foreground hover:text-foreground"><X className="w-5 h-5" /></button>
        </div>
