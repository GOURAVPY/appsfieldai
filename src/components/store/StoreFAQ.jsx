...
export default function StoreFAQ({ faqs = [], title, brandColor = "#f97316" }) {
  const [openIndex, setOpenIndex] = useState(null);
  const items = (faqs || []).filter((f) => f?.question && f?.answer);
  if (items.length === 0) return null;

  return (
    <section id="store-faq" className="border-t border-border/40 bg-background">
      <div className="max-w-3xl mx-auto px-6 py-14">
        <div className="text-center mb-8">
          ...
          <h2 className="text-2xl sm:text-3xl font-display font-bold">{title || "Frequently Asked Questions"}</h2>
        </div>
