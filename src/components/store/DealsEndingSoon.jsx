...
export default function DealsEndingSoon({ listings = [], onViewDetails, onReserveSpot }) {
        ...
        {deals.map((l, i) => (
          <motion.div key={l.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }} className="snap-start shrink-0 w-[280px]">
            <div className="relative">
              ...
              <SaaSCard listing={l} delay={0} onViewDetails={onViewDetails} onBuySpot={onViewDetails} onReserveSpot={onReserveSpot || onViewDetails} />
            </div>
          </motion.div>
        ))}
      </div>
