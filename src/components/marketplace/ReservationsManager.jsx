...
import { CalendarCheck, CheckCircle, Ban, Phone, TrendingUp, BadgeCheck, Trash2, Mail, Copy, Check, DollarSign, MessageSquare } from "lucide-react";
...
const statusConfig = {
  pending: { color: "bg-amber-500/10 text-amber-400 border-amber-500/20", icon: CalendarCheck, label: "Pending" },
...
export default function ReservationsManager() {
  ...
  const handleStatusChange = async (reservation, newStatus) => {
    await base44.entities.DealReservations.update(reservation.id, { status: newStatus });
    ...
    try {
      await base44.entities.Notification.create({
        ...
        title: `Reservation ${newStatus === "approved" ? "Approved" : newStatus === "rejected" ? "Rejected" : "Updated"}`,
        message: `Your spot reservation for "${reservation.listingTitle || "the listing"}" has been ${newStatus.replace("_", " ")}.`,
        listingId: reservation.listingId,
        relatedRequestId: reservation.id,
        isRead: false,
      });
    } catch (_) {}
  ...
  const handleDelete = async (reservation) => {
    await base44.entities.DealReservations.delete(reservation.id);
    queryClient.invalidateQueries({ queryKey: ["allReservations"] });
  ...
  const getListingName = (r) => {
    if (r.listingTitle) return r.listingTitle;
        ...
        <Card className="border-border/40 bg-[#1a1a1a]">
          ...
          <CardContent className="divide-y divide-border/20">
            {isLoading ? (
              ...
              <p className="text-sm text-muted-foreground py-8 text-center">No reservations yet</p>
            ) : (
              reservations.map((r) => (
                  ...
                  <div className="min-w-0 flex-1 space-y-1.5">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <div className="flex items-center gap-0.5 ml-1">
                        {r.userEmail && (
                          <>
                            <a href={`mailto:${r.userEmail}`} className="p-1 rounded hover:bg-secondary/50 text-muted-foreground hover:text-foreground">
                              <Mail className="w-3 h-3" />
                            </a>
                            <button
                              onClick={() => handleCopy(`re-${r.id}`, r.userEmail)}
                              className="p-1 rounded hover:bg-secondary/50 text-muted-foreground hover:text-foreground"
                            >
                              {copied[`re-${r.id}`] ? <Check className="w-3 h-3 text-emerald-400" /> : <Copy className="w-3 h-3" />}
                            </button>
                          </>
                        )}
                      </div>
                    </div>
                    <p className="text-xs text-violet-400">{getListingName(r)}</p>
                    ...
                    {r.status === "pending" && (
                      <>
                        ...
                        <Button
                          size="sm"
                          variant="ghost"
                          onClick={() => handleStatusChange(r, "contacted")}
                    ...
                    {r.status === "approved" && (
                      <>
                        ...
                        <Button
                          size="sm"
                          variant="ghost"
                          onClick={() => handleStatusChange(r, "deal_in_progress")}
                    ...
                    {r.status === "contacted" && (
                      <>
                        ...
                        <Button
                          size="sm"
                          variant="ghost"
                          onClick={() => handleStatusChange(r, "deal_closed")}
